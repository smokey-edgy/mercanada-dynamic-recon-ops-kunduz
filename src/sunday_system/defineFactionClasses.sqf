...
_advancedGroundFaction = ((_this select 1) select 0);
_advancedAirFaction = ((_this select 1) select 1);
_advancedSeaFaction = ((_this select 1) select 2);
...
_infClassesUnarmed = [];
_infClassesUnarmedForWeights = [];
_infClassUnarmedWeights = [];
...

{
	...
	if (((configFile >> "CfgVehicles" >> (configName _x) >> "faction") call BIS_fnc_GetCfgData) == _faction) then {		
		if (((configFile >> "CfgVehicles" >> (configName _x) >> "scope") call BIS_fnc_GetCfgData) == 2) then {	
				...
				if ( ["officer", (configName _x), false] call BIS_fnc_inString ) then {
					...
					if (
						...
						if ((count ((configFile >> "CfgVehicles" >> (configName _x) >> "weapons") call BIS_fnc_GetCfgData) <= 2)) then {
							_infClassesUnarmed pushBack (configName _x);
							_infClassesUnarmedForWeights pushBack (configName _x);
							_infClassUnarmedWeights pushBack 0.5;							
						} else {						
			...
			if ((configName _x isKindOf 'StaticMGWeapon') || (configName _x isKindOf 'StaticGrenadeLauncher')) then {
				_staticClasses pushBack (configName _x);
			};			
		...
		if (count _advancedGroundFaction > 0) then {
				if (((configFile >> "CfgVehicles" >> (configName _x) >> "faction") call BIS_fnc_GetCfgData) == _advancedGroundFaction) then {
					if (configName _x isKindOf 'Car') then {
					_edSubcat = ((configFile >> "CfgVehicles" >> (configName _x) >> "editorSubcategory") call BIS_fnc_GetCfgData);
					if (!isNil "_edSubcat") then {
						if ( ["EdSubcat_Drones", ((configFile >> "CfgVehicles" >> (configName _x) >> "editorSubcategory") call BIS_fnc_GetCfgData), false] call BIS_fnc_inString ) then {
		...
		if (count _advancedAirFaction > 0) then {
			if (((configFile >> "CfgVehicles" >> (configName _x) >> "faction") call BIS_fnc_GetCfgData) == _advancedAirFaction) then {				
				_edSubcat = ((configFile >> "CfgVehicles" >> (configName _x) >> "editorSubcategory") call BIS_fnc_GetCfgData);
				if (!isNil "_edSubcat") then {					
					if ( ["EdSubcat_Helicopters", ((configFile >> "CfgVehicles" >> (configName _x) >> "editorSubcategory") call BIS_fnc_GetCfgData), false] call BIS_fnc_inString ) then {
						_heliClasses pushBack (configName _x);
		...
		if (count _advancedSeaFaction > 0) then {
			if (((configFile >> "CfgVehicles" >> (configName _x) >> "faction") call BIS_fnc_GetCfgData) == _advancedSeaFaction) then {				
				_edSubcat = ((configFile >> "CfgVehicles" >> (configName _x) >> "editorSubcategory") call BIS_fnc_GetCfgData);
				if (!isNil "_edSubcat") then {					
					if ( ["EdSubcat_Boats", ((configFile >> "CfgVehicles" >> (configName _x) >> "editorSubcategory") call BIS_fnc_GetCfgData), false] call BIS_fnc_inString ) then {
						_shipClasses pushBack (configName _x);
...
if (count _infClassesUnarmed > count _infClasses) then {
	_infClasses = _infClasses + _infClassesUnarmed;
	_infClassesForWeights = _infClassesForWeights + _infClassesUnarmedForWeights;
	_infClassWeights = _infClassWeights + _infClassUnarmedWeights;
};
