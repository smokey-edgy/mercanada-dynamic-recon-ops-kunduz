...
_minefieldPlaces = [];
{
	if ((_x select 1) > 0.5) then {
		...
		_nearPeople = (_x select 0) nearEntities ["Man", 200];
		{
...
if (count _minefieldPlaces > 0) then {
	...
	for "_i" from 1 to ([1,3] call BIS_fnc_randomInt) step 1 do {
		...
		_grid = [_thisPos, 10, 10, 15] call sun_defineGrid;
		
		_markerName = format["mineMkr%1", floor(random 10000)];
		_markerSizeX = ([60,120] call BIS_fnc_randomInt);
		_markerSizeY = ([60,120] call BIS_fnc_randomInt);
