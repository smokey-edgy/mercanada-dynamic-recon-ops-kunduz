	...
	{	
		...
		if (_civInteractType < 50) then {
			...
			if (!isNil "_civChosen") then {
				_enemyPos = [(getPos _civChosen), 2, _civDir] call dro_extendPos;
...
if (aiSkill == 2) then {
	_numInf = [2,3] call BIS_fnc_randomInt;
};
...
for "_infIndex" from 1 to _numInf do {
	_infPosition = [];
	if (_infIndex <= 2) then {_infPosition = selectRandom AO_groundPosClose} else {_infPosition = selectRandom AO_groundPosFar};	
	_spawnedSquad = nil;
