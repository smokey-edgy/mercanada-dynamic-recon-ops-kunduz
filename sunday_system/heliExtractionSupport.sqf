...
if (count _heliTransports > 0) then {
	...
	_providerPos = [getPos trgAOC,6000,8000,0,1,1,0] call BIS_fnc_findSafePos;
	...
	//Setup provider values	
	{
		...
		["BIS_SUPP_vehicleinit","diag_log _this; _this setCaptive true; _this setBehaviour 'CARELESS';"],	
		["BIS_SUPP_filter","FACTION"]		
	];	
	
	[requester, "Transport", 1] call BIS_fnc_limitSupport;
	
	[u1, requester, providerTransport] call BIS_fnc_addSupportLink;
};